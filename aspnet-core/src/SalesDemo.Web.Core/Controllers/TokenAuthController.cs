...
using SalesDemo.Authentication.TwoFactor.Google;
using SalesDemo.Authorization;
using SalesDemo.Authorization.Accounts.Dto;
...
namespace SalesDemo.Web.Controllers
{
    ...
    public class TokenAuthController : SalesDemoControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(SalesDemoConsts.DateTimeOffsetFormat));
